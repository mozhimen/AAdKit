...
class NativeAdActivity : com.mozhimen.adk.topon.test.bases.BaseActivity(), View.OnClickListener {
    ...
    private fun getNativeAdTypeFromIntent(): String {
        return getIntent().getStringExtra("native_type") ?: com.mozhimen.adk.topon.test.annors.AAdNativeType.NATIVE_SELF_RENDER_TYPE
    }
...
//        NativeAd nativeAd = mATNative.getNativeAd();
        val nativeAd = mATNative!!.getNativeAd(com.mozhimen.adk.topon.test.cons.AdConst.SCENARIO_ID.NATIVE_AD_SCENARIO)
        if (nativeAd != null) {
            if (mNativeAd != null) {
                mNativeAd!!.destory()
            }
    ...
    override fun onClick(v: View?) {
        ...
        when (v.id) {
            ...
            R.id.show_ad_btn -> {
                ATNative.entryAdScenario(mCurrentPlacementId, com.mozhimen.adk.topon.test.cons.AdConst.SCENARIO_ID.NATIVE_AD_SCENARIO)
                if (isAdReady()) {
                    showAd()
                }
            }
