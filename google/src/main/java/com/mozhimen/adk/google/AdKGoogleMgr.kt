...
//    }
    ...
    fun init(context: Context, listener: IA_Listener<Boolean>) {
        MobileAds.initialize(context) { initializationStatus ->
            ...
            for (adapterStatusName in adapterStatuses.keys) {
                val status = adapterStatuses[adapterStatusName]
                UtilKLogWrapper.d(TAG, "Adapter name: $adapterStatusName, Description: ${status?.description}, Latency: ${status?.latency}")
            }
            val readyAdapters: List<MutableMap.MutableEntry<String, AdapterStatus>> = initializationStatus.adapterStatusMap.entries.filter {
                // 判断适配器初始化的状态
            ...
            if (readyAdapters.isNotEmpty()) {
                // 适配器的名称
                val adapterNames = readyAdapters.map { it.key }.list2str()
                UtilKLogWrapper.d(TAG, "初始化: success adapterNames $adapterNames")
