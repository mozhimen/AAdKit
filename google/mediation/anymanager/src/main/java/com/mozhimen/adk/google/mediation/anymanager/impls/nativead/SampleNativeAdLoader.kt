...
import com.mozhimen.adk.google.mediation.anymanager.R
 ...
 */
class SampleNativeAdLoader constructor(private val context: Context) {
     ...
     */
    fun fetchAd(request: SampleNativeAdRequest) {
    ...
    private fun createSampleAppInstallAd(request: SampleNativeAdRequest): SampleNativeAd {
        ...
        if ((Random()).nextInt(100) < 80) {
            nativeAd.setMediaView(SampleMediaView(context))
        } else {
    ...
    private fun createSampleContentAd(request: SampleNativeAdRequest): SampleNativeAd {
        val nativeAd: SampleNativeAd = SampleNativeAd()
